# MONOLITH LIQUIDATION ENGINE — COMPREHENSIVE AUDIT PRIMER v0.3
...
### HF Formula (Single-Collateral Overcollateralization Model) — UPDATED v0.3
...
Where:
- **Collateral Value** = assetShares[user] × assetPrice × assetIndex / PRECISION (single collateral only in v0.3)
...
### Partial Liquidation Flow (Single-Collateral) — UPDATED v0.3
...
### Vulnerability MON-L-003: Partial Liquidation Dust Trapping (ENHANCED v0.3)
...
### Vulnerability MON-L-004: Liquidation Bonus Overflow → Negative Seizure (ENHANCED v0.3)
...
### Vulnerability MON-L-005: Accrual Window Race → Stale Liquidation Price (ENHANCED v0.3)
...
## SANDWICH & MEV ATTACKS (NEW v0.3 SECTION)

### Vulnerability MON-L-006: Sandwich Redemption → Collateral Theft (NEW)
...
## INTEREST-FREE & REDEMPTION-FREE MODE EFFECTS (NEW v0.3 SECTION)
...
### Vulnerability MON-L-007: Interest-Free Mode Liquidation Underaccrual (NEW)
...
## YIELD VAULT LIQUIDATION EFFECTS (NEW v0.3 SECTION)

### Vulnerability MON-L-008: Collateral Yield Draining During Liquidation (NEW)
...
## REBASE & ORACLE TIMING (NEW v0.3 SECTION)

### Vulnerability MON-L-009: Liquidation Mispricing via Rebase-Oracle Misalignment (NEW)
...
### Vulnerability MON-L-010: Stale Oracle + High Liquidation Threshold = False Insolvency (NEW)
...
## INVARIANT CATALOG (LIQUIDATION MODULE v0.3)
...
| INV-L-008 | Liquidation uses current debtIndex | Accrual race, stale debt pricing | **NEW** |
| INV-L-009 | All debt mutations preceded by accrual | Interest undercount, liquidator underpay | **NEW** |
| INV-L-010 | Liquidation is atomic, no partial redemption during | Sandwich redemption, double-spending | **NEW** |
| INV-L-011 | Collateral not redeemable during liquidation of same amount | MEV collateral extraction | **NEW** |
| INV-L-012 | Liquidation accrues catch-up debt for interest-free users | Underaccrual, attacker gain | **NEW** |
| INV-L-013 | Liquidation bonus ≤ repayment value + yield bonus | Yield draining during liquidation | **NEW** |
| INV-L-014 | Yield not drained during liquidation (cleared beforehand) | Collateral over-seizure | **NEW** |
| INV-L-015 | Liquidation price synchronizes with collateral rebase state | Mispricing via rebase desync | **NEW** |
| INV-L-016 | Oracle freshness proportional to liquidationThreshold | Aggressive risk models exploit | **NEW** |
| INV-L-017 | Minimum repayment amount enforced per liquidation | Dust trapping via spam | **NEW** |
| INV-L-018 | Liquidation checks borrower mode (REDEMPTION_FREE blocks liquidation) | Mode-bypass attacks | **NEW** |

---

## FOUNDRY TEST SKELETONS (LIQUIDATION v0.3)
...
- MON-L-006: Sandwich Redemption → Collateral Theft
- MON-L-007: Interest-Free Mode Liquidation Underaccrual
- MON-L-008: Collateral Yield Draining During Liquidation
- MON-L-009: Liquidation Mispricing via Rebase-Oracle Misalignment
- MON-L-010: Stale Oracle + High Liquidation Threshold = False Insolvency

**Added 8 new invariants:**
- INV-L-010 through INV-L-017 (Sandwich atomicity, borrower modes, yield drainage, oracle-rebase sync, staleness thresholds)

**Expanded sections:**
- Single-Collateral HF Adjustments (v0.3): Simplified formula, numerical examples
- Accrual & Liquidation Race Conditions (ENHANCED): Debt underflow detailed
- Interest-Free & Redemption-Free Mode Effects (NEW): Liquidation impact analysis
- Yield Vault Liquidation Effects (NEW): Yield drainage, fee evasion
- Rebase & Oracle Timing (NEW): Misalignment risks, staleness threshold tuning

**Added Foundry test skeletons:**
- Skeleton 2: Sandwich & Atomicity (2 test cases)
- Skeleton 3: Borrower Mode + Yield (3 test cases)

**Added numerical examples:**
- Single-collateral HF calculations
- Sandwich redemption collateral theft quantification
- Yield drainage cost estimation
- Rebase-oracle misalignment compound effect
